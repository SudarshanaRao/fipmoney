...
import { 
  ...
  Activity, CircleDollarSign, Percent, X, Check, Search, Bell, Eye, Lock, RefreshCw, BarChart2, Info, ArrowUp, Crown, LineChart, Banknote, Calendar, ChevronDown, Clock, ChevronLeft
} from "lucide-react";
...
export default function PortfolioPage({ onNavigate }: PortfolioPageProps) {
  ...
  return (
    <div className="flex-1 h-screen overflow-y-auto bg-[#fcfdfd] pb-24 relative font-sans text-gray-800">
      ...
      <div className="relative z-10 p-4 sm:p-6 md:p-8 lg:p-10 max-w-7xl mx-auto space-y-6 md:space-y-8">
        
        {/* ========================================================================= */}
        {/* LAPTOP / LARGE SCREENS VIEW (EXACT REFERENCE CODE LAYOUT) */}
        {/* ========================================================================= */}
        ...
        {/* ========================================================================= */}
        {/* MOBILE SCREENS VIEW (CUSTOM SIMPLIFIED DESIGN) */}
        {/* ========================================================================= */}
        <div className="block md:hidden space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6">
             {/* PORTFOLIO CARD */}
             <div className="bg-[#1a1525] rounded-[24px] p-6 relative overflow-hidden text-white flex flex-col justify-between shadow-[0_8px_30px_rgba(0,0,0,0.12)] min-h-[260px]">
               <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-600/10 blur-3xl rounded-full pointer-events-none z-0" />
               <div className="absolute bottom-0 left-0 w-64 h-64 bg-purple-900/20 blur-3xl rounded-full pointer-events-none z-0" />
               <svg className="absolute inset-0 w-full h-full pointer-events-none z-0" preserveAspectRatio="none" viewBox="0 0 500 250">
                  <path d="M0,180 C100,180 180,80 250,130 C350,200 420,100 500,80 L500,250 L0,250 Z" fill="url(#wave-grad-mob)" opacity="0.15" />
                  <path d="M0,180 C100,180 180,80 250,130 C350,200 420,100 500,80" fill="none" stroke="#818cf8" strokeWidth="1.5" strokeOpacity="0.35" />
                  <defs>
               ...
               <div className="relative z-10">
                  ...
                  <div className="text-[36px] font-black text-white tracking-tight leading-none mb-3">
                     ₹{Math.round(totalValue).toLocaleString()}
                  </div>
                  <div className="flex items-center gap-2">
                     <span className="flex items-center gap-1 bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded text-[11px] font-bold border border-emerald-500/20">
               ...
               <div className="relative z-10 grid grid-cols-2 gap-6 mt-8 pt-6 border-t border-white/10">
                  <div className="flex flex-col gap-1.5 border-r border-white/10 pr-4">
                     <div className="flex items-center gap-2 text-[11px] text-gray-300 font-medium">
                        <div className="w-6 h-6 rounded-full bg-amber-500/10 flex items-center justify-center border border-amber-500/20">
                           <img src="/gold-bars.png" alt="Digital Gold" className="w-3.5 h-3.5 object-contain" />
                        </div>
                        Digital Gold
                     </div>
                     <div className="text-[16px] font-bold ml-8 text-white">₹{Math.round(goldValue).toLocaleString()}</div>
                  </div>
                  <div className="flex flex-col gap-1.5 pl-2">
                     <div className="flex items-center gap-2 text-[11px] text-gray-300 font-medium">
                        <div className="w-6 h-6 rounded-full bg-purple-500/10 flex items-center justify-center border border-purple-500/20">
                           <img src="/silver.png" alt="Digital Silver" className="w-3.5 h-3.5 object-contain" />
                        </div>
                        Digital Silver
                     </div>
                     <div className="text-[16px] font-bold ml-8 text-white">₹{Math.round(silverValue).toLocaleString()}</div>
                  </div>
               </div>
             </div>
             ...
             <div className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm">
                <div className="flex justify-between items-center mb-6">
                   <h3 className="text-[15px] font-bold text-gray-900">Performance Summary</h3>
                   <div className="flex items-center gap-1 px-3 py-1.5 bg-gray-50 rounded-lg text-[11px] font-medium text-gray-600 border border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors">
                      Today <ChevronDown size={12}/>
                   </div>
                </div>
                <div className="space-y-4">
                   <div className="flex justify-between items-center text-[13px]">
                      <span className="text-gray-500 font-medium">Today's Gain</span>
                      <span className="text-emerald-500 font-bold flex items-center gap-1"><ArrowUp size={12} strokeWidth={3}/> ₹1,250 (1.12%)</span>
                   </div>
                   <div className="w-full h-px bg-gray-50" />
                   <div className="flex justify-between items-center text-[13px]">
                      <span className="text-gray-500 font-medium">Total Gain</span>
                      <span className="text-emerald-500 font-bold flex items-center gap-1"><ArrowUp size={12} strokeWidth={3}/> ₹{Math.round(Math.abs(totalGain)).toLocaleString()} ({totalGainPercent.toFixed(2)}%)</span>
                   </div>
                   <div className="w-full h-px bg-gray-50" />
                   <div className="flex justify-between items-center text-[13px]">
                      <span className="text-gray-500 font-medium">All Time Returns</span>
                      <span className="text-emerald-500 font-bold flex items-center gap-1"><ArrowUp size={12} strokeWidth={3}/> 12.48%</span>
                   </div>
                   <div className="w-full h-px bg-gray-50" />
                   <div className="flex justify-between items-center text-[13px]">
                      <span className="text-gray-500 font-medium">Invested Amount</span>
             ...
             {/* ASSET ALLOCATION */}
             <div className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm flex flex-col justify-between min-h-[260px]">
                <h3 className="text-[15px] font-bold text-gray-900 mb-6">Asset Allocation</h3>
                ...
                <div className="flex-1 flex flex-row items-center justify-between gap-4">
                   ...
                   <div className="relative w-32 h-32 shrink-0">
                      <svg viewBox="0 0 38 38" className="w-full h-full transform -rotate-90">
                         <circle cx="19" cy="19" r="15.9155" fill="transparent" stroke="#f1f5f9" strokeWidth="6" />
                         <circle cx="19" cy="19" r="15.9155" fill="transparent" stroke="#a855f7" strokeWidth="6" strokeDasharray={`${silverAlloc} ${100 - silverAlloc}`} strokeDashoffset="0" />
                         <circle cx="19" cy="19" r="15.9155" fill="transparent" stroke="#fbbf24" strokeWidth="6" strokeDasharray={`${goldAlloc} ${100 - goldAlloc}`} strokeDashoffset={-silverAlloc} />
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center bg-white m-5 rounded-full shadow-inner">
                         <span className="text-[9px] font-bold text-gray-400 uppercase">Total</span>
                         <span className="text-[16px] font-black text-gray-900">100%</span>
                      </div>
                   </div>
                   
                   {/* Legend Text Beside Piechart */}
                   <div className="flex-1 space-y-3 w-full">
                      <div className="flex justify-between items-center bg-amber-50/50 p-2.5 rounded-xl border border-amber-100/60">
                         <div className="flex items-center gap-2">
                            ...
                            <div>
                               <div className="text-[12px] font-bold text-gray-900 leading-none">Digital Gold</div>
                               <div className="text-[10px] text-gray-500 font-medium mt-1">₹{Math.round(goldValue).toLocaleString()}</div>
                            </div>
                         </div>
                         <span className="text-[12px] font-extrabold text-amber-600 bg-white px-2 py-0.5 rounded-md border border-amber-200/50">{goldAlloc.toFixed(1)}%</span>
                      </div>

                      <div className="flex justify-between items-center bg-purple-50/50 p-2.5 rounded-xl border border-purple-100/60">
                         <div className="flex items-center gap-2">
                            <div className="w-2.5 h-2.5 rounded-full bg-purple-500 shrink-0" />
                            <div>
                               <div className="text-[12px] font-bold text-gray-900 leading-none">Digital Silver</div>
                               <div className="text-[10px] text-gray-500 font-medium mt-1">₹{Math.round(silverValue).toLocaleString()}</div>
                            </div>
                         </div>
                         <span className="text-[12px] font-extrabold text-purple-600 bg-white px-2 py-0.5 rounded-md border border-purple-200/50">{silverAlloc.toFixed(1)}%</span>
                      </div>
                   </div>
                </div>
          ...
          <div className="mt-8 mb-6">
             <h2 className="text-[20px] font-bold text-gray-900 mb-4 tracking-tight">Your Holdings</h2>
             
             <div className="bg-white rounded-[28px] p-6 border border-gray-100 shadow-sm space-y-6">
                
                {/* DIGITAL GOLD ROW */}
                <div className="flex flex-col justify-between gap-4">
                   <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-full bg-amber-50/80 flex items-center justify-center p-2.5 shrink-0 border border-amber-100/50">
                         <img src="/gold-bars.png" alt="Digital Gold" className="w-9 h-9 object-contain drop-shadow-sm" />
                      </div>
                      
                      <div>
                         <div className="text-[13px] font-semibold text-gray-600 mb-0.5">Digital Gold</div>
                         <div className="text-[22px] font-extrabold text-gray-900 leading-tight">
                            {goldHoldings.toFixed(4)} g
                         </div>
                         <div className="text-[11px] text-gray-400 font-medium mt-0.5">
                            Avg. Buy Price ₹{avgGoldBuyPrice.toFixed(2)}/g
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                      <div>
                         <div className="text-[11px] text-gray-400 font-medium mb-0.5">Current Value</div>
                         <div className="text-[15px] font-extrabold text-gray-900">
                            ₹{Math.round(goldValue).toLocaleString()}
                         </div>
                      </div>

                      <div className="text-right">
                         <div className="text-[11px] text-gray-400 font-medium mb-0.5">Total Returns</div>
                         <div className="text-[13px] font-extrabold text-emerald-600 flex items-center justify-end gap-1">
                            <ArrowUp size={12} strokeWidth={3} /> {(((goldValue - goldCostBasis) / (goldCostBasis || 1)) * 100).toFixed(2)}%
                         </div>
                      </div>
                   </div>
                ...
                <div className="flex flex-col justify-between gap-4">
                   <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-full bg-purple-50/80 flex items-center justify-center p-2.5 shrink-0 border border-purple-100/50">
                         <img src="/silver.png" alt="Digital Silver" className="w-9 h-9 object-contain drop-shadow-sm" />
                      </div>
                      
                      <div>
                         <div className="text-[13px] font-semibold text-gray-600 mb-0.5">Digital Silver</div>
                         <div className="text-[22px] font-extrabold text-gray-900 leading-tight">
                            {silverHoldings.toFixed(4)} g
                         </div>
                         <div className="text-[11px] text-gray-400 font-medium mt-0.5">
                            Avg. Buy Price ₹{avgSilverBuyPrice.toFixed(2)}/g
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                      <div>
                         <div className="text-[11px] text-gray-400 font-medium mb-0.5">Current Value</div>
                         <div className="text-[15px] font-extrabold text-gray-900">
                            ₹{Math.round(silverValue).toLocaleString()}
                         </div>
                      </div>
                      ...
                      <div className="text-right">
                         <div className="text-[11px] text-gray-400 font-medium mb-0.5">Total Returns</div>
                         <div className="text-[13px] font-extrabold text-emerald-600 flex items-center justify-end gap-1">
                            <ArrowUp size={12} strokeWidth={3} /> {(((silverValue - silverCostBasis) / (silverCostBasis || 1)) * 100).toFixed(2)}%
                         </div>
                      </div>
                   </div>
          ...
          <div className="mt-8 mb-10">
             <div className="flex justify-between items-center mb-4">
                <h2 className="text-[20px] font-bold text-gray-900 tracking-tight">Recent Transactions</h2>
                <button 
                  onClick={() => onNavigate && onNavigate("history")}
                  className="text-[13px] font-bold text-amber-600 hover:text-amber-700 flex items-center gap-1 transition-colors cursor-pointer bg-transparent border-none outline-none"
                >
                  View Transactions <ChevronRight size={16} />
                </button>
             </div>

             <div className="bg-white rounded-[28px] p-6 border border-gray-100 shadow-sm divide-y divide-gray-100">
                {recentTxs && recentTxs.length > 0 ? (
                   recentTxs.slice(0, 4).map((tx) => (
                      <div key={tx.id} className="py-3.5 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
                         <div className="flex items-center gap-3.5">
                            <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${
                               tx.category === "Gold" ? "bg-amber-50 border border-amber-100" :
                               tx.category === "Silver" ? "bg-purple-50 border border-purple-100" :
                               "bg-blue-50 border border-blue-100"
                            }`}>
                               {tx.category === "Gold" ? (
                                  <img src="/gold-bars.png" alt="Gold" className="w-6 h-6 object-contain" />
                               ) : tx.category === "Silver" ? (
                            ...
                            <div>
                               ...
                               <div className="text-[11px] text-gray-400 font-medium">
                                  {new Date(tx.date).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })} · {tx.source || tx.paymentMethod}
                               </div>
                            </div>
                         </div>

                         <div className="text-right">
                            <div className={`text-[15px] font-extrabold ${tx.type === "Buy" ? "text-emerald-600" : "text-gray-900"}`}>
                               {tx.type === "Buy" ? "+" : "-"}₹{tx.amount ? tx.amount.toLocaleString('en-IN') : "0"}
                            </div>
                            {tx.grams && (
                   ...
                   /* Sample Demo Transactions when empty */
                   [
                      { id: 'demo1', type: 'Buy', category: 'Gold', amount: 5000, grams: '0.7788 g', date: new Date().toISOString(), status: 'Completed', source: 'Digital Gold Vault' },
                      { id: 'demo2', type: 'Buy', category: 'Silver', amount: 2500, grams: '29.6912 g', date: new Date(Date.now() - 86400000 * 2).toISOString(), status: 'Completed', source: 'Digital Silver Vault' },
                      { id: 'demo3', type: 'Sell', category: 'Gold', amount: 1200, grams: '0.1869 g', date: new Date(Date.now() - 86400000 * 5).toISOString(), status: 'Completed', source: 'Bank Instant Transfer' }
                   ].map((tx) => (
                      <div key={tx.id} className="py-3.5 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
                         <div className="flex items-center gap-3.5">
                            <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${
                               tx.category === "Gold" ? "bg-amber-50 border border-amber-100" : "bg-purple-50 border border-purple-100"
                            }`}>
                               <img src={tx.category === "Gold" ? "/gold-bars.png" : "/silver.png"} alt={tx.category} className="w-6 h-6 object-contain" />
                            </div>
                            <div>
                               <div className="text-[14px] font-bold text-gray-900">
                                  {tx.type} {tx.category}
                               ...
                               <div className="text-[11px] text-gray-400 font-medium">
                                  {new Date(tx.date).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })} · {tx.source}
                               </div>
                            </div>
                         </div>
