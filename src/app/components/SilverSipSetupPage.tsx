...
interface SilverSipSetupPageProps {
  initialPlan?: PlanFrequency;
  onNavigate: (page: string) => void;
}

const ANNUAL_SILVER_RETURN_RATE = 0.12; // ~12% historical silver CAGR

export default function SilverSipSetupPage({ initialPlan = 'daily', onNavigate }: SilverSipSetupPageProps) {
  ...
  const liveGramPrice = 98; // 999 Fine Silver Rate per gram

  // Calculation Logic for Silver
  const calculationData = useMemo(() => {
    let periodsPerYear = 12;
    if (frequency === 'daily') periodsPerYear = 365;
    if (frequency === 'weekly') periodsPerYear = 52;

    const ratePerPeriod = ANNUAL_SILVER_RETURN_RATE / periodsPerYear;
  ...
  return (
    <div className="min-h-screen bg-sky-50/40 pb-20 pt-3 sm:pt-6 font-sans">
      <div className="max-w-xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-bold text-slate-700 bg-white border border-sky-200/80 px-3.5 py-2 rounded-xl shadow-2xs hover:bg-sky-50 cursor-pointer outline-none"
          >
            <ArrowLeft size={16} />
            <span>Back to Silver Savings</span>
          </button>

          <span className="bg-sky-100 text-sky-900 border border-sky-300/80 px-3 py-1 rounded-xl text-xs font-black flex items-center gap-1.5">
            <img src="/silver.png" alt="Silver Bars" className="w-4 h-4 object-contain" />
        ...
        <motion.div
          ...
          className="space-y-4"
        >
              <div className="bg-white border border-sky-200/80 rounded-3xl p-5 sm:p-7 shadow-sm space-y-6">
                
                {/* Title */}
                <div className="flex items-center gap-3">
                  <img
                    src={frequency === 'daily' ? '/silver_daily.png' : frequency === 'weekly' ? '/silver_weekly.png' : '/silver_monthly.png'}
                    alt="Silver SIP Plan"
                    className="w-10 h-10 object-contain shrink-0 drop-shadow-xs"
                  />
                  <div>
                    <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
                      <span>999 Silver SIP Setup</span>
                    </h1>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2 bg-sky-50/80 p-1.5 rounded-2xl border border-sky-200/60">
                    {(['daily', 'weekly', 'monthly'] as PlanFrequency[]).map((f) => {
                      const active = frequency === f;
                      const iconSrc = f === 'daily' ? '/silver_daily.png' : f === 'weekly' ? '/silver_weekly.png' : '/silver_monthly.png';
                      return (
                        <button
                          key={f}
                          type="button"
                          onClick={() => setFrequency(f)}
                          className={`py-2.5 px-2 rounded-xl text-xs capitalize transition-all cursor-pointer border-none outline-none flex items-center justify-center gap-1.5 ${
                            active
                              ? 'bg-sky-600 text-white shadow-xs font-black'
                              : 'text-slate-700 hover:text-slate-900 font-semibold'
                          }`}
                        >
                          <img src={iconSrc} alt={f} className="w-4 h-4 object-contain" />
                          <span>{f}</span>
                        </button>
                      );
                    })}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-sky-600 text-lg">₹</span>
                    <input
                      type="number"
                      min={10}
                      value={amount}
                      onChange={(e) => setAmount(Math.max(1, Number(e.target.value)))}
                      className="w-full bg-sky-50/30 border border-sky-200 rounded-2xl pl-9 pr-4 py-3 font-black text-slate-900 text-xl focus:outline-none focus:border-sky-500 transition-all"
                    />
                  </div>
                      ...
                      ? [10, 20, 50, 100, 200, 500].map((amt) => (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer border-none outline-none ${
                              amount === amt ? 'bg-sky-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-sky-100'
                            }`}
                          >
                            ₹{amt}
                          </button>
                      ...
                      {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day) => (
                        <button
                          ...
                          className={`py-2 rounded-xl text-xs font-bold cursor-pointer border-none outline-none ${
                            weeklyDay === day ? 'bg-sky-600 text-white' : 'bg-slate-100 text-slate-700'
                          }`}
                        >
                          {day.slice(0, 3)}
                        </button>
                ...
                {frequency === 'monthly' && (
                  <div className="space-y-2 relative">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs font-black text-sky-700 bg-sky-100 px-2.5 py-0.5 rounded-md">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between bg-sky-50/60 border border-sky-200/90 rounded-2xl px-4 py-3 text-xs font-black text-slate-800 hover:bg-sky-100/60 transition-all cursor-pointer outline-none"
                    >
                    ...
                    <AnimatePresence>
                      {isCalendarOpen && (
                        <motion.div
                          ...
                          className="absolute left-0 right-0 z-30 bg-white border border-sky-200/90 rounded-2xl p-3 shadow-xl space-y-2 mt-1"
                        >
                              ...
                              return (
                                <button
                                  ...
                                  className={`h-9 rounded-xl font-bold text-xs flex items-center justify-center transition-all cursor-pointer border-none outline-none ${
                                    isSelected
                                      ? 'bg-sky-600 text-white font-black shadow-xs scale-105'
                                      : 'bg-slate-50 text-slate-700 hover:bg-sky-100 border border-slate-200/60'
                                  }`}
                                >
                                  {dateNum}
                                </button>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-bold text-sky-700">{tenureYears} Years</span>
                  </div>
                ...
                {/* Silver Return Projections */}
                <div className="bg-gradient-to-br from-slate-900 via-sky-950 to-slate-950 text-white rounded-2xl p-5 space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="text-[11px] font-bold text-sky-200">Expected Silver Value ({tenureYears} Yrs)</div>
                      <div className="text-2xl sm:text-3xl font-black text-white mt-0.5">
                        ₹{calculationData.totalValue.toLocaleString()}
                      </div>
                      <div className="text-[11px] font-bold text-sky-300 mt-0.5">
                        ~{calculationData.estimatedGrams} Grams 999 Silver
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-sky-800/40">
                    <div>
                      <span className="text-sky-200/80">Invested: </span>
                      <span className="font-bold">₹{calculationData.totalInvested.toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="text-sky-200/80">Est. Gains: </span>
                ...
                <button
                  ...
                  className="w-full py-4 rounded-2xl bg-sky-600 hover:bg-sky-700 text-white font-black text-sm shadow-md flex items-center justify-center gap-2 cursor-pointer border-none outline-none transition-all"
                >
                  <span>Confirm Silver SIP Mandate</span>
                  <ArrowRight size={16} />
                </button>
        ...
        {/* Silver SIP Success Modal Popup */}
        <AnimatePresence>
          {showSuccessModal && (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <motion.div
                ...
                className="w-full max-w-md bg-white rounded-[32px] p-6 shadow-2xl space-y-5 border border-sky-200/80 text-center relative overflow-hidden my-auto"
              >
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500 mt-1">
                    Your 999 Fine Digital Silver SIP is configured and automated.
                  </p>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 text-left space-y-2.5 text-xs">
                  <div className="flex justify-between items-center pb-2 border-b border-slate-200/60">
                    ...
                    <span className="font-black text-sky-700 flex items-center gap-1.5">
                      <img
                        src={frequency === 'daily' ? '/silver_daily.png' : frequency === 'weekly' ? '/silver_weekly.png' : '/silver_monthly.png'}
                        alt="Silver SIP"
                        className="w-5 h-5 object-contain"
                      />
                      <span>999 Fine Digital Silver</span>
                    </span>
                  </div>
                  <div className="flex justify-between items-center pb-2 border-b border-slate-200/60">
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    onClick={() => onNavigate('silver-savings')}
                    className="flex-1 py-3.5 rounded-2xl bg-sky-600 hover:bg-sky-700 text-white font-black text-xs cursor-pointer border-none outline-none transition-all shadow-md"
                  >
                    View Silver SIPs
                  </button>
                  <button
